...
func nodeGpuTaint() (selector map[string]string, taints []corev1.Toleration)  {
...
func nodeCpuTaint() (taints []corev1.Toleration, affinity corev1.Affinity){
	tolerations := []corev1.Toleration{
		{
			Key:"servertype",
			Operator: corev1.TolerationOpEqual,
	...
	nodeAffity := corev1.NodeAffinity{
		RequiredDuringSchedulingIgnoredDuringExecution: &nodeSelectors,
	}
	affinities := corev1.Affinity{
		NodeAffinity: &nodeAffity,
	}
	return tolerations, affinities
...
func newLauncher(mpiJob *kubeflow.MPIJob, kubectlDeliveryImage string) *batchv1.Job {
    ...
    selector, taints := nodeGpuTaint()
    podSpec.Spec.NodeSelector = selector
    podSpec.Spec.Tolerations = taints
